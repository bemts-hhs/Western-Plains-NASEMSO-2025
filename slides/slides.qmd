...
# get counts
sex_counts <- trauma_data_clean_iowa |>
...
# get counts with annual trends
race_counts <- trauma_data_clean_iowa |>
  dplyr::mutate(
    Patient_Race = ifelse(
      is.na(Patient_Race) |
...
# get counts with annual trends
non_white_race_counts <- trauma_data_clean_iowa |>
  dplyr::mutate(
    Patient_Race = ifelse(
      is.na(Patient_Race) |
        grepl(pattern = "not\\s|select", x = Patient_Race, ignore.case = TRUE),
...
insurance_counts <- trauma_data_clean_iowa |>
  dplyr::mutate(
...
# capture counts and preserve ordering by desc(n)
body_region_counts <- trauma_data_clean_iowa |>
  dplyr::mutate(
...
overall_tta_rate <- trauma_data_clean_iowa |>
...
# capture counts and preserve ordering by desc(n)
tta_counts <- trauma_data_clean_iowa |>
...
falls_iss_counts <- trauma_data_clean_iowa |>
...
mortality_rates <- trauma_data_clean_iowa |>
...
rmm_overall <- trauma_data_clean_iowa |>
...
rmm_summary <- trauma_data_clean_iowa |>
  dplyr::filter(!is.na(Probability_of_Survival_Calc), !is.na(Death)) |>
  dplyr::mutate(Survival = ifelse(Death, FALSE, TRUE)) |>
  dplyr::distinct(Unique_Incident_ID, .keep_all = TRUE) |>
...
saved_lives <- trauma_data_clean_iowa |>
